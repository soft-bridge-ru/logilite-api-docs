...
<!-- Контент из api.md -->
## Описание API интеграции с АБ Логистик v25.8.25
...
В документе `index.md` в разделе **3.3.2. Поле `Упаковка`** текущее описание слишком краткое и противоречит более точной структуре, представленной в файле `Интеграция с АБЛогистик_v25.8.1.docx`. Ниже приведено **скорректированное и уточнённое описание объекта `Упаковка`**, соответствующее спецификации из документа v25.8.1, с добавлением примера и пояснений.
...
#### ✅ Пример для товара без упаковки:
```json
"Упаковка": ""
```
...
> 💡 Пустое значение результата подтверждает успешный запрос в базу РЦ и очистку последующих ответов от данной записи в возвращаемом методом `getdata` результате.
