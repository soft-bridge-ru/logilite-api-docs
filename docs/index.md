...
#### Поле `УсловиеХранения`
| Поле | Тип | Описание | Обязательное |
|------|-----|----------|--------------|
| УсловиеХранения | Строка | Температурный или иной режим хранения товара | Нет |

> ⚠️ ***Примечание:*** Значение передаётся как строка. Рекомендуется использовать стандартизированные формулировки, согласованные с клиентом и РЦ. Точные допустимые значения могут быть ограничены справочником на стороне 1С.

***Допустимые значения (рекомендуемые шаблоны):***
- `"18+-5"` — хранение при комнатной температуре (13–23 °C)
- `"4+-2"` — охлаждённое хранение (2–6 °C)
- `"-18"` — замороженное хранение (не выше -18 °C)
...
***Список возможных значений (на основе стандартного реквизита `ЕдиницаИзмеренияСрокаГодности` карточки Номенклатура в 1С):***
